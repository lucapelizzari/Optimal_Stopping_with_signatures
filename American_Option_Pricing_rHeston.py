...
def LongstaffSchwartz_signature_rHeston(M,M2,N,N1,T,phi,rho,K,KK_primal,X0,H,lam,nu,theta,V_0,r,N_approx):
    ...
    """
    if N_approx == 3:
        nodes = np.array([1/30,2.2416,46.831])
        weights = np.array([0.55543,1.1110,6.0858])
    elif N_approx == 2:
        nodes = np.array([0.05,8.7171])
        weights = np.array([0.76733,3.2294])
    s = time.time()
    #number of strikes
    N_strikes = len(phi)
    #Step 1: First simulation of rHeston price and vol using Simons Algo.
    S = samples(lam, nu, theta, V_0, T, nodes, weights, rho, X0, r, M, N,sample_paths=True)
    X = S[0][0,:,:].transpose()
    V = S[0][1,:,:].transpose()
    ...
    del X,V,Basis,Basis_Reg,Basis_primal,P_primal,S,SS,QV,dX
    #Start Resimulation
    S = samples(lam, nu, theta, V_0, T, nodes, weights, rho, X0, r, M2, N,sample_paths=True)
    X = S[0][0,:,:].transpose()
    V = S[0][1,:,:].transpose()
    #exercise-dates with and without zero
    #Payoff-process on finer grid
